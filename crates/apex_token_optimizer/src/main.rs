...
fn effort_efficiency(total: f64, waste: f64) -> f64 {
    if total <= 0.0 {
        ...
        effort_valid(total, waste) / total
    }
}
...
fn usage() {
    eprintln!("usage:");
    eprintln!(
        "  apex_token_optimizer correct --x 100 --y 50 --sw 1920 --sh 1080 --iw 1000 --ih 500"
    );
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "reserve" => {
            ...
            println!(
                "{{\"token_reserve\":{},\"kept_frames\":{},\"dropped_frames\":{}}}",
                total,
                imgs.len().min(keep),
                dropped
            );
        ...
        "effort" => {
            ...
            println!(
                "{{\"effort_valid\":{:.3},\"efficiency\":{:.6}}}",
                effort_valid(total, waste),
                effort_efficiency(total, waste)
            );
        }
        "purify" => {
