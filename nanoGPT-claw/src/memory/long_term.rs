...
impl LongTermMemory {
    ...
    pub fn add_memory(&mut self, content: String, importance: f32, tags: Vec<String>) {
        ...
        // 触发语义聚类更新
        if self.memories.len().is_multiple_of(10) {
    ...
    fn update_stats(&mut self) {
        ...
        if !self.memories.is_empty() {
            let total_importance: f32 = self.memories.values().map(|m| m.importance).sum();
    ...
    pub fn get_by_importance(&self, min_importance: f32) -> Vec<MemoryEntry> {
        ...
        self.by_importance
            .range(key..)
            .flat_map(|(_, ids)| ids.iter().filter_map(|id| self.memories.get(id)).cloned())
            .collect()
    }
}
