...
use crate::skill::{Skill, SkillCategory, SkillError, SkillMetadata, SkillRegistry, SkillResult};
...
impl Skill for CargoCheckSkill {
    ...
    async fn execute(&self, _params: HashMap<String, String>) -> Result<SkillResult, SkillError> {
        ...
        let output = Command::new("cargo")
            ...
            .output()
            .map_err(|e| {
                SkillError::ExecutionFailed(format!("Failed to run cargo check: {}", e))
            })?;
...
impl Skill for CargoClippySkill {
    ...
    async fn execute(&self, _params: HashMap<String, String>) -> Result<SkillResult, SkillError> {
        ...
        let output = Command::new("cargo")
            ...
            .output()
            .map_err(|e| {
                SkillError::ExecutionFailed(format!("Failed to run cargo clippy: {}", e))
            })?;
        ...
        if output.status.success() {
            ...
            Ok(SkillResult {
                success: false,
                output: format!(
                    "❌ cargo clippy found {} warnings in {}ms:\n{}",
    ...
    pub fn new() -> Self {
        Self {
            metadata: SkillMetadata {
                ...
                category: SkillCategory::Code,
                enabled: true,
                parameters: vec![crate::skill::SkillParameter {
