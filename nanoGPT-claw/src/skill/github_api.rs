use crate::skill::{Skill, SkillCategory, SkillError, SkillMetadata, SkillResult};
            ...
            metadata: SkillMetadata {
                ...
                parameters: vec![
                    crate::skill::SkillParameter {
                        ...
                        description:
                            "操作类型: create-issue, list-issues, trigger-actions, create-pr"
                                .to_string(),
...
impl GitHubApiSkill {
    fn get_token(&self) -> Result<String, String> {
        std::env::var("GITHUB_TOKEN").map_err(|_| "GITHUB_TOKEN 环境变量未设置".to_string())
    }
    ...
    fn get_repo_info(&self) -> Result<(String, String), String> {
        ...
        if parts.len() >= 2 {
            ...
            Err(
                "无法解析仓库信息: 请设置 GITHUB_REPO_OWNER 和 GITHUB_REPO_NAME 环境变量"
                    .to_string(),
            )
        }
    }
    ...
    async fn auto_evolve(&self) -> Result<String, String> {
        ...
        params.insert(
            "title".to_string(),
            format!("[Auto] 进化报告 #{}", chrono::Utc::now().timestamp()),
        );
        params.insert("body".to_string(), issue_body);
        params.insert(
            "labels".to_string(),
            "auto-evolve,gpt-generated".to_string(),
        );
