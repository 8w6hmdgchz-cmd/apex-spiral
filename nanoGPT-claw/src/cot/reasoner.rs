...
use crate::cot::{CoTConfig, CoTError, CoTResult, ReasoningType, ThoughtStep};
...
impl Reasoner {
    ...
    pub async fn reason(&self, problem: &str) -> Result<CoTResult, CoTError> {
        ...
        info!(
            "Starting REAL CoT reasoning for: {}...",
            Self::truncate(problem, 50)
        );
        ...
        for step_num in 0..self.config.max_steps {
            ...
            if current_context.len() > MAX_CONTEXT_LENGTH {
                warn!(
                    "Context too long, truncating to {} chars",
                    MAX_CONTEXT_LENGTH
                );
            ...
            if thought_step.confidence >= self.config.confidence_threshold {
                info!(
                    "Reached confidence threshold ({:.2}) at step {}",
    ...
    async fn execute_step(
        ...
        let llm_response = self
            .scheduler
            .submit_to_main(&prompt)
            .await
            .map_err(|e| CoTError::InferenceFailed(e.to_string()))?;

        debug!(
            "LLM response for step {}: {}",
            step_index,
            Self::truncate(&llm_response, 100)
        );
    ...
    async fn generate_final_conclusion(
        &self,
        problem: &str,
        steps: &[ThoughtStep],
