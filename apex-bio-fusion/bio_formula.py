...
if __name__ == "__main__":
    # 基线参数
    Lambda, Theta, K, xi, Psi, Phi, H, T, epsilon = 0.85, 0.90, 0.80, 0.75, 0.95, 0.70, 0.45, 1.20, 0.60
    
    # 不带生物调控
    delta_g_base = calculate_delta_g_with_bio(Lambda, Theta, K, xi, Psi, Phi, H, T, epsilon)
    print(f"ΔG (baseline): {delta_g_base:.3f}")
    
    # 带生物调控
    bio = BioParams(EMF=0.7, Ca=0.6, Gene_reg=0.8, Loss_bio=0.4, tau=1.0)
    delta_g_bio = calculate_delta_g_with_bio(Lambda, Theta, K, xi, Psi, Phi, H, T, epsilon, bio)
    print(f"ΔG (with Θ_bio): {delta_g_bio:.3f}")
    print(f"提升: {(delta_g_bio/delta_g_base - 1)*100:.1f}%")