...
def main() -> None:
    ...
    # FIX R10 bug-029: absorbed.list 含 separator 不一致 (pyg-team/pytorch/geometric
    # vs pyg-team/pytorch_geometric) 与大小写不一致 (Reflexion vs reflexion) 的同仓库
    # 多 entry. setdefault 只按精确字符串去重, 导致 unique_repos 虚高 ~3. 改用
    # normalize_key (lowercase + slash→underscore) 合并, 让 unique_repos 反映真 unique.
    # 副作用: A_net_breakdown 也会合并, A_net 数值变化小 (因 p*u 累加是按 entry 算的,
    # 合并后会少 ~3 个低贡献项, A_net 略降 0.05-0.1, F_hunt/A_n_norm 不受影响).
    def _normalize_repo_key(k: str) -> str:
        return k.lower().replace('/', '_').replace('-', '_')
    norm_repos: dict[str, dict[str, object]] = {}
    for repo_key, meta in repos.items():
        nk = _normalize_repo_key(repo_key)
        if nk not in norm_repos:
            norm_repos[nk] = {'name': meta['name'], 'keywords': set(), 'orig_keys': set()}
        norm_repos[nk]['keywords'] |= meta['keywords']
        norm_repos[nk]['orig_keys'].add(repo_key)
    repos = norm_repos  # 后续代码用 repos, 自动走去重后的 norm dict

    breakdown = {}
    for repo, meta in sorted(repos.items()):
        # 还原为 cache 目录命名查找 README (用 norm key 的任一 orig_key 试)
        cache_path = None
        for ok in meta['orig_keys']:
            cp = CACHE / ok.replace('/', '_') / 'README.md'
            if cp.exists():
                cache_path = cp
                break
        p_value, u_value = WEIGHTS.get(repo, (0.65, 0.65))
        keyword_bonus = min(len(meta['keywords']), 5) * 0.015
        cache_bonus = 0.02 if cache_path is not None else 0
