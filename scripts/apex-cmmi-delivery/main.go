...
func run(root,obj,mode string)Report{
	...
	phases:=[]Phase{
		...
		{"P5","GitHub Release Sync","governance","CMMI CM + DAR","safe rebase/push",[]string{"memory/metrics/task_runs.jsonl"},"planned"},
	}
	status:="success"
	...
	return Report{ID:fmt.Sprintf("apex-cmmi-%d",time.Now().Unix()),StartedAt:time.Now().Format(time.RFC3339),Objective:obj,Status:status,ContainerMode:container,DockerAvailable:docker,Formula:"Apex_CMMI = Apex_agent × (Plan→Code→Audit→Test→Release) × EvidenceGate",Phases:phases,Evidence:[]string{"state/apex-cmmi-delivery-latest.json","state/apex-fusion-engine-latest.json","state/apex-fusion-evidence-report.json"},Next:"use_as_default_delivery_harness",Format:"apex-cmmi-delivery-1.0"}
}
