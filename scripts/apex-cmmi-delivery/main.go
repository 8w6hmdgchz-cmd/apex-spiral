...
func run(root,obj,mode string)Report{
	...
	phases:=[]Phase{
		...
		{"P3","Claude-Code Implementation Slot","coding agent","CMMI TS + PI","git diff + build gate",[]string{"scripts/apex-agent-dispatch/main.go"},"planned"},
	...
	return Report{ID:fmt.Sprintf("apex-cmmi-%d",time.Now().Unix()),StartedAt:time.Now().Format(time.RFC3339),Objective:obj,Status:status,ContainerMode:container,DockerAvailable:docker,Formula:"Apex_CMMI = Apex_agent × (Container→Plan→Code→Audit→Test→Memory→Release) × EvidenceGate",Phases:phases,Evidence:[]string{"state/apex-container-backend-latest.json","state/apex-cmmi-delivery-latest.json","state/apex-fusion-engine-latest.json","state/apex-fusion-evidence-report.json","state/apex-memory-admission-latest.json"},Next:"use_as_default_delivery_harness",Format:"apex-cmmi-delivery-1.0"}
}
