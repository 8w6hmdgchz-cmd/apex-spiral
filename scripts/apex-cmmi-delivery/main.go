...
func run(root,obj,mode string)Report{
	...
	phases:=[]Phase{
		{"P1","APEX Formula Planning","GPT/phasor planner","CMMI PP + IPM","phasor route + twelve-factor gate",[]string{"state/apex-phasor-llm-latest.json","state/apex-12factor-agent-latest.json"},"planned"},
		{"P2","Claude-Code Implementation Slot","coding agent","CMMI TS + PI","git diff + build gate",[]string{"scripts/apex-agent-dispatch/main.go"},"planned"},
		{"P3","APEX PR Audit","GPT/APEX reviewer","CMMI VER + VAL","fusion + evidence validator",[]string{"state/apex-fusion-engine-latest.json","state/apex-fusion-evidence-report.json"},"planned"},
		{"P4","Automated Test Closure","harness","CMMI PPQA + CM","ECC cycle + 12factor + hygiene",[]string{"state/apex-ecc-runtimeos-latest.json","state/apex-hygiene-latest.json"},"planned"},
		{"P5","Evidence Memory Admission","memory governor","CMMI MA + PPQA","evidence validator + sigma memory admission",[]string{"state/apex-memory-admission-latest.json","state/apex-memory-admission-evidence-report.json"},"planned"},
		{"P6","GitHub Release Sync","governance","CMMI CM + DAR","safe rebase/push",[]string{"memory/metrics/task_runs.jsonl"},"planned"},
	}
	status:="success"
	...
	return Report{ID:fmt.Sprintf("apex-cmmi-%d",time.Now().Unix()),StartedAt:time.Now().Format(time.RFC3339),Objective:obj,Status:status,ContainerMode:container,DockerAvailable:docker,Formula:"Apex_CMMI = Apex_agent × (Plan→Code→Audit→Test→Memory→Release) × EvidenceGate",Phases:phases,Evidence:[]string{"state/apex-cmmi-delivery-latest.json","state/apex-fusion-engine-latest.json","state/apex-fusion-evidence-report.json","state/apex-memory-admission-latest.json"},Next:"use_as_default_delivery_harness",Format:"apex-cmmi-delivery-1.0"}
}
